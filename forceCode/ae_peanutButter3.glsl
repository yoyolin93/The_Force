...
void main(){
    ...
    float centerDist = distance(camPos, center);
    if(centerDist > 0.2){
        float speed = (1.-quant(centerDist, 25.)) *  sin(time/100.) *5.3;
        camPos = wrap(rotate(camPos, center, time/10. * speed), 0., 1.);
    }
    vec3 camRot = texture2D(channel0, camPos).xyz;
 ...
 gl_FragColor = vec4(vec3(camRot), 1);
