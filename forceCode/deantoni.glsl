...
vec2 videoLayerWarp(vec2 stN, vec3 c, int quantMode, float quantNum, float t2){
    ...
    for(float i = 0.; i < 5.; i++){
        ...
        if(quantMode == 2) {
            ...
            float dist = 1. - sigmoid(distance(warp2, pole) * quantNum * scale);
        ...
        if(quantMode == 2) {
            ...
            float dist = 1. - sigmoid(distance(warp1, pole) * quantNum * scale);
...
void main() {
    ...
    vec2 warp = videoLayerWarp(stN, c, 1, 20., time);
