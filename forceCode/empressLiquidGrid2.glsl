...
void main () {
    ...
    vec2 deepTileWave = rowColWave(camN, 1. + sinN(t1) * 2., t2, power);
    vec2 warpTileCoord2 = wrap(rotate(deepTileWave, vec2(0.5), rw)*(5.*sinN(stime*2.)+1.), 0., 1.);
    vec3 wrapTile = texture2D(channel5, warpTileCoord2).rgb;
