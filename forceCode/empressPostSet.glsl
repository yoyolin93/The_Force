...
vec3 kscope(vec2 stN){
    ...
    vec3 scopec = texture2D( channel5, uv*vec2(1,-1)+.5, -1.0).rgb;
    return scopec;
}
...
void main () {
    ...
    vec2 deepTileWave = rowColWave(camN * (.2 + sinN(t0)*3.), 1. + sinN(t1) * 20., t2, power);
    ...
    if(SONG==BALDGIRL) ballInd = noteColorBalls(warpQ, time*2.);
    ...
    if(ballInd == -1){ 
        ...
        if(SONG == PARTYTRIX) ballColor = wrap(ballColor*(1.+sinN(time)*10.), 0., 1.);
