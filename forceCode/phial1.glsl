...
void main () {
    ...
    blend = mouseK.x / 1.;
    // blend = 0.;
    stN = mix(stN, quant(cam.xy, 5.), pow(sinN(time/5.), 3.));
    ...
    vec2 coord =  5. * rotate(stN, vec2(0.5), time/10.) * rotate(stN, vec2(sinN(time+stN.x), cosN(time+stN.y)), time/20.);
    float div =  200. + sinN(t1) * 300.;
    ...
    float numStripes = 3. +  sinN(time/7.)*12.;(mouseK.y == 0. ? 7. : pow(mouseK.y, 2.5) * 20.);
    float d = 0.05;
