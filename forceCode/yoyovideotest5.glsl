...
void ex3() {
    ...
    vec2 colorN = texture2D(channel5, nn).rg;
    ...
    vec2 dev1 = rotate(vec2(0., time/4. + sinN(time*3.+colorN.x*3.)), vec2(0.5), 0.);
    vec2 dev2 = rotate(mod(vec2(0., time/4. + sinN(time*3.+rotN.x*3.)), 1.), vec2(0.5), time);
    vec2 dev = quant(dev2, 5. * mod(rotN.x + time/3.7, 1.));
    vec2 idk1 = mix(mod(stN+dev, 1.), stN, sinN(time+rotN.x*4.));
    
    modN = idk1;
    
    vec3 vidnoise = texture2D(channel5, modN).rgb;
    vec3 vid = texture2D(channel5, nn).rgb;
    ...
    vec3 final = vidnoise;
    // final  = feedback > 0.2 ? vidnoise : vid;
